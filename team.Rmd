...
[**Link to Repository**](https://github.com/xazip/ds202_lab2)
...
Company_Highest_Mean_CocoaPct <- aggregate(data = choco, choco$Cocoa.Pct, by = list(Company = choco$Company), FUN = mean) # Read up on aggregate using stack overflow and https://www.rdocumentation.org/packages/stats/versions/3.5.2/topics/aggregate. 
...
# This project utilized using git to colaborate on a project with another partner.  Instead of submitting individual Rmd files, we were working with one Rmd file.  The workflow was pretty balanced. We met after class to discuss the project overview. We then worked remotely from our own seperate devices. I was doing the coding.  My partner wrote the descriptions of my codes. There were some minor git bash issues along the way.  We were able to work around these issues and work as a team remotely from our own computers.  I believe this was the main idea of lab 2 and I believe we both did a good job. 